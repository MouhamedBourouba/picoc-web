...
void StdioFprintfLong(StdOutStream *Stream, const char *Format,
                      uint64_t Value) {
  ...
  while (*Format) {
    ...
    case 'X':
      UseFormat = PRIX64;
      break;
    /* Ignore the %l (long) specifier, because of course we're doing longs in
     * this function */
    case 'l':
      break;
    default:
...
#ifndef WIN32
    int CCount =
        snprintf(Stream->StrOutPtr, Stream->StrOutLen, PlatformFormat, Value);
#else
    int CCount =
        _snprintf(Stream->StrOutPtr, Stream->StrOutLen, PlatformFormat, Value);
#endif
    Stream->StrOutPtr += CCount;
    Stream->StrOutLen -= CCount;
    Stream->CharCount += CCount;
...
void StdioFprintfPointer(StdOutStream *Stream, const char *Format,
                         void *Value) {
  if (Stream->FilePtr != NULL)
    Stream->CharCount += fprintf(Stream->FilePtr, Format, Value);
  else if (Stream->StrOutLen >= 0) {
        ...
        if (ArgCount >= Args->NumArgs)
          ...
          if (ShowType == &pc->LongType) {
            ...
            if (IS_NUMERIC_COERCIBLE(ThisArg))
              StdioFprintfLong(&SOStream, OneFormatBuf,
                               ThisArg->Val->LongInteger);
            else
              StdioOutPuts("XXX", &SOStream);
            ...
            if (IS_NUMERIC_COERCIBLE(ThisArg))
              StdioFprintfLong(&SOStream, OneFormatBuf,
                               ThisArg->Val->UnsignedLongInteger);
            else
              StdioOutPuts("XXX", &SOStream);
            ...
            if (IS_NUMERIC_COERCIBLE(ThisArg))
              StdioFprintfWord(
                  &SOStream, OneFormatBuf,
                  (unsigned int)ExpressionCoerceUnsignedInteger(ThisArg));
            else
              StdioOutPuts("XXX", &SOStream);
...
int StdioBaseScanf(struct ParseState *Parser, FILE *Stream, char *StrIn,
  ...
  if (Stream != NULL)
    return fscanf(Stream, Format, ScanfArg[0], ScanfArg[1], ScanfArg[2],
                  ScanfArg[3], ScanfArg[4], ScanfArg[5], ScanfArg[6],
                  ScanfArg[7], ScanfArg[8], ScanfArg[9]);
  else
...
void StdioFopen(struct ParseState *Parser, struct Value *ReturnValue,
  ...
  ReturnValue->Val->Pointer =
      fopen(Param[0]->Val->Pointer, Param[1]->Val->Pointer);
}

void StdioFreopen(struct ParseState *Parser, struct Value *ReturnValue,
                  struct Value **Param, int NumArgs) {
  ReturnValue->Val->Pointer = freopen(
...
void StdioFread(struct ParseState *Parser, struct Value *ReturnValue,
  ...
  ReturnValue->Val->Integer =
      fread(Param[0]->Val->Pointer, Param[1]->Val->Integer,
...
void StdioFwrite(struct ParseState *Parser, struct Value *ReturnValue,
                 struct Value **Param, int NumArgs) {
  ReturnValue->Val->Integer =
      fwrite(Param[0]->Val->Pointer, Param[1]->Val->Integer,
             Param[2]->Val->Integer, Param[3]->Val->Pointer);
}
...
void StdioRename(struct ParseState *Parser, struct Value *ReturnValue,
                 struct Value **Param, int NumArgs) {
  ReturnValue->Val->Integer =
      rename(Param[0]->Val->Pointer, Param[1]->Val->Pointer);
}
...
void StdioFseek(struct ParseState *Parser, struct Value *ReturnValue,
                struct Value **Param, int NumArgs) {
  ReturnValue->Val->Integer = fseek(
      Param[0]->Val->Pointer, Param[1]->Val->Integer, Param[2]->Val->Integer);
}
...
void StdioPutc(struct ParseState *Parser, struct Value *ReturnValue,
               struct Value **Param, int NumArgs) {
  ReturnValue->Val->Integer =
      putc(Param[0]->Val->Integer, Param[1]->Val->Pointer);
}
...
void StdioUngetc(struct ParseState *Parser, struct Value *ReturnValue,
                 struct Value **Param, int NumArgs) {
  ReturnValue->Val->Integer =
      ungetc(Param[0]->Val->Integer, Param[1]->Val->Pointer);
}
...
void StdioGets(struct ParseState *Parser, struct Value *ReturnValue,
               struct Value **Param, int NumArgs) {
  ReturnValue->Val->Pointer =
      fgets(Param[0]->Val->Pointer, GETS_MAXValue, stdin);
...
void StdioPrintf(struct ParseState *Parser, struct Value *ReturnValue,
  ...
  ReturnValue->Val->Integer = StdioBasePrintf(
      Parser, stdout, NULL, 0, Param[0]->Val->Pointer, &PrintfArgs);
}

void StdioVprintf(struct ParseState *Parser, struct Value *ReturnValue,
                  struct Value **Param, int NumArgs) {
  ReturnValue->Val->Integer = StdioBasePrintf(
      Parser, stdout, NULL, 0, Param[0]->Val->Pointer, Param[1]->Val->Pointer);
}
...
void StdioFprintf(struct ParseState *Parser, struct Value *ReturnValue,
  ...
  ReturnValue->Val->Integer =
      StdioBasePrintf(Parser, Param[0]->Val->Pointer, NULL, 0,
                      Param[1]->Val->Pointer, &PrintfArgs);
}
...
void StdioSprintf(struct ParseState *Parser, struct Value *ReturnValue,
  ...
  PrintfArgs.NumArgs = NumArgs - 2;
  ReturnValue->Val->Integer =
      StdioBasePrintf(Parser, NULL, Param[0]->Val->Pointer, -1,
                      Param[1]->Val->Pointer, &PrintfArgs);
}
...
void StdioSnprintf(struct ParseState *Parser, struct Value *ReturnValue,
  ...
  ReturnValue->Val->Integer = StdioBasePrintf(
      Parser, NULL, Param[0]->Val->Pointer, Param[1]->Val->Integer,
      Param[2]->Val->Pointer, &PrintfArgs);
}
...
void StdioFscanf(struct ParseState *Parser, struct Value *ReturnValue,
  ...
  ReturnValue->Val->Integer = StdioBaseScanf(
      Parser, Param[0]->Val->Pointer, NULL, Param[1]->Val->Pointer, &ScanfArgs);
}
...
void StdioSscanf(struct ParseState *Parser, struct Value *ReturnValue,
  ...
  ScanfArgs.NumArgs = NumArgs - 2;
  ReturnValue->Val->Integer = StdioBaseScanf(
      Parser, NULL, Param[0]->Val->Pointer, Param[1]->Val->Pointer, &ScanfArgs);
}
...
void StdioVscanf(struct ParseState *Parser, struct Value *ReturnValue,
                 struct Value **Param, int NumArgs) {
  ReturnValue->Val->Integer = StdioBaseScanf(
      Parser, stdin, NULL, Param[0]->Val->Pointer, Param[1]->Val->Pointer);
}

void StdioVfscanf(struct ParseState *Parser, struct Value *ReturnValue,
                  struct Value **Param, int NumArgs) {
  ReturnValue->Val->Integer =
      StdioBaseScanf(Parser, Param[0]->Val->Pointer, NULL,
                     Param[1]->Val->Pointer, Param[2]->Val->Pointer);
}
...
void StdioVsscanf(struct ParseState *Parser, struct Value *ReturnValue,
                  struct Value **Param, int NumArgs) {
  ReturnValue->Val->Integer =
      StdioBaseScanf(Parser, NULL, Param[0]->Val->Pointer,
                     Param[1]->Val->Pointer, Param[2]->Val->Pointer);
}
...
void StdioSetupFunc(Picoc *pc) {
  ...
  StructFileType =
      TypeCreateOpaqueStruct(pc, NULL,
                             // CHANGED
                             TableStrRegister(pc, "__FILEStruct"), 0);
...
void PrintCh(char OutCh, FILE *Stream) { putc(OutCh, Stream); }

void PrintSimpleInt(long Num, FILE *Stream) { fprintf(Stream, "%ld", Num); }

void PrintStr(const char *Str, FILE *Stream) { fputs(Str, Stream); }

void PrintFP(double Num, FILE *Stream) { fprintf(Stream, "%f", Num); }
