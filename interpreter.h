...
#define IS_INTEGER_NUMERIC_TYPE(t)                                             \
  ((t)->Base >= TypeInt && (t)->Base <= TypeUnsignedLong)
#define IS_INTEGER_NUMERIC(v) IS_INTEGER_NUMERIC_TYPE((v)->Typ)
#define IS_NUMERIC_COERCIBLE(v) (IS_INTEGER_NUMERIC(v) || IS_FP(v))
#define IS_NUMERIC_COERCIBLE_PLUS_POINTERS(v, ap)                              \
  (IS_NUMERIC_COERCIBLE(v) || IS_POINTER_COERCIBLE(v, ap))
...
struct ValueType {
  ...
  struct ValueType
      *DerivedTypeList;   /* first in a list of types derived from this one */
...
struct Value {
  ...
  union AnyValue
      *Val; /* pointer to the AnyValue which holds the actual content */
  struct Value *LValueFrom; /* if an LValue, this is a Value our LValue is
                               contained within (or NULL) */
  char ValOnHeap;           /* this Value is on the heap */
  char ValOnStack;   /* the AnyValue is on the stack along with this Value */
  char AnyValOnHeap; /* the AnyValue is separately allocated from the Value on
                        the heap */
  char IsLValue;     /* is modifiable and is allocated somewhere we can usefully
                        modify it */
...
struct Picoc_Struct {
  ...
  struct AllocNode
      *FreeListBucket[FREELIST_BUCKETS]; /* we keep a pool of freelist buckets
...
extern void LexInitParser(struct ParseState *Parser, Picoc *pc,
                          const char *SourceText, void *TokenSource,
                          char *FileName, int RunIt, int SetDebugMode);
extern enum LexToken LexGetToken(struct ParseState *Parser,
                                 struct Value **Value, int IncPos);
extern enum LexToken LexRawPeekToken(struct ParseState *Parser);
extern void LexToEndOfMacro(struct ParseState *Parser);
extern void *LexCopyTokens(struct ParseState *StartParser,
                           struct ParseState *EndParser);
 ...
 * void PicocParse(const char *FileName, const char *Source, int SourceLen, int
 * RunIt, int CleanupNow, int CleanupSource); void PicocParseInteractive(); */
...
extern void ExpressionAssign(struct ParseState *Parser, struct Value *DestValue,
                             struct Value *SourceValue, int Force,
...
extern struct ValueType *TypeGetMatching(Picoc *pc, struct ParseState *Parser,
                                         ...
                                         enum BaseType Base, int ArraySize,
                                         const char *Identifier,
                                         int AllowDuplicates);
extern struct ValueType *TypeCreateOpaqueStruct(Picoc *pc,
                                                struct ParseState *Parser,
                                                const char *StructName,
                                                int Size);
extern int TypeIsForwardDeclared(struct ParseState *Parser,
                                 struct ValueType *Typ);
...
extern void *VariableAlloc(Picoc *pc, struct ParseState *Parser, int Size,
                           int OnHeap);
extern void VariableStackPop(struct ParseState *Parser, struct Value *Var);
extern struct Value *
VariableAllocValueAndData(Picoc *pc, struct ParseState *Parser, int DataSize,
                          int IsLValue, struct Value *LValueFrom, int OnHeap);
extern struct Value *VariableAllocValueAndCopy(Picoc *pc,
                                               struct ParseState *Parser,
                                               struct Value *FromValue,
                                               int OnHeap);
extern struct Value *
VariableAllocValueFromType(Picoc *pc, struct ParseState *Parser,
                           struct ValueType *Typ, int IsLValue,
                           struct Value *LValueFrom, int OnHeap);
extern struct Value *VariableAllocValueFromExistingData(
    struct ParseState *Parser, struct ValueType *Typ, union AnyValue *FromValue,
    int IsLValue, struct Value *LValueFrom);
...
extern struct Value *VariableDefine(Picoc *pc, struct ParseState *Parser,
                                    char *Ident, struct Value *InitValue,
                                    struct ValueType *Typ, int MakeWritable);
...
extern void VariableDefinePlatformVar(Picoc *pc, struct ParseState *Parser,
                                      ...
                                      int IsWritable);
extern void VariableStackFrameAdd(struct ParseState *Parser,
                                  const char *FuncName, int NumParams);
...
extern void VariableStringLiteralDefine(Picoc *pc, char *Ident,
                                        struct Value *Val);
...
extern void VariableScopeEnd(struct ParseState *Parser, int ScopeID,
                             int PrevScopeID);
...
extern void AssignFail(struct ParseState *Parser, const char *Format,
                       struct ValueType *Type1, struct ValueType *Type2,
